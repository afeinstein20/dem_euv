...
def generate_gj436_data_npy():
    fname = 'hlsp_muscles_cxo_acis_gj436_na_v22_component-spec.fits'
    ...
    plt.show()
    np.save('gj436_spectrum_data.npy',
            [xray_wave, xray_bins,
             xray_flux, xray_err])
    return 'gj436_spectrum_data.npy'


if __name__ == '__main__':
    temp = np.logspace(4, 8, 2000)
    log_temp = np.log10(temp)
    star_dist = 9.7559 * u.pc
    star_rad = 0.432 * u.R_sun
    star_name_root = 'gj436'
    abundance = -0.04
    abund_type = 'sol0'
    ...
    init_pos = [22.49331207, -3.31678227, -0.49848262,
                ...
                -0.08085897]
    plt.semilogx(temp,
                 np.polynomial.chebyshev.chebval(np.linspace(-1, 1, 2000),
                                                 init_pos[:-1]))
    plt.show()
    ...
    if os.path.isfile('gj436_spectrum_data.npy'):
        data_npy_file = 'gj436_spectrum_data.npy'
    else:
        data_npy_file = generate_gj436_data_npy()
    line_table_file = 'gj436_linetable.ascii'
