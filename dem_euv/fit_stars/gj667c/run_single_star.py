...
def generate_gj667c_data_npy():
    fname = 'hlsp_muscles_cxo_acis_gj667c_na_v22_component-spec.fits'
    ...
    plt.show()
    np.save('gj667c_spectrum_data.npy',
            [xray_wave, xray_bins,
             xray_flux, xray_err])
    return 'gj667c_spectrum_data.npy'


if __name__ == '__main__':
    temp = np.logspace(4, 8, 2000)
    log_temp = np.log10(temp)
    star_dist = 7.2454 * u.pc
    star_rad = 0.337 * u.R_sun
    star_name_root = 'gj667c'
    abundance = -0.3
    ...
    if os.path.isfile('gj667c_spectrum_data.npy'):
        data_npy_file = 'gj667c_spectrum_data.npy'
    else:
        data_npy_file = generate_gj667c_data_npy()
    line_table_file = 'gj667c_linetable.ascii'
