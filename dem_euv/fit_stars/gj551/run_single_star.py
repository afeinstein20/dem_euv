...
def generate_gj551_data_npy():
    fname = 'hlsp_muscles_xmm_epic_gj551_multi_v22_component-spec.fits'
    xray_fits = fits.open(fname)

    xray_wave = xray_fits[1].data['WAVELENGTH']
    xray_flux = xray_fits[1].data['FLUX']
    xray_err = xray_fits[1].data['ERROR']

    xray_flux = xray_flux[np.argsort(xray_wave)]
    xray_err = xray_err[np.argsort(xray_wave)]
    xray_wave = np.sort(xray_wave)
    print(xray_wave, np.diff(xray_wave))
    plt.plot(xray_wave[1:], xray_wave[1:] / np.diff(xray_wave))
    plt.show()
    _, xray_flux = bintoR(xray_wave, xray_flux, R=80)
    xray_wave, xray_var = bintoR(xray_wave, xray_err**2, R=80)
    ...
    plt.show()
    np.save('gj551_spectrum_data.npy',
            [xray_wave, xray_bins,
             xray_flux, xray_err])
    return 'gj551_spectrum_data.npy'


if __name__ == '__main__':
    temp = np.logspace(4, 8, 2000)
    log_temp = np.log10(temp)
    star_dist = 1.3012 * u.pc
    star_rad = 0.147 * u.R_sun
    star_name_root = 'gj551'
    abundance = 0.0
    ...
    if os.path.isfile('gj551_spectrum_data.npy'):
        data_npy_file = 'gj551_spectrum_data.npy'
    else:
        data_npy_file = generate_gj551_data_npy()
    line_table_file = 'gj551_linetable.ascii'
